...
### Room Detail
Visualizes booked studio time by site and day, driven by the Master Scheduler report — screen-only, not printed. Richmond always sorts first and is the default site. Richmond's columns are a curated, stable set — every room tagged `physical` in Admin → Rooms always shows, even with nothing booked there that day — plus any extra room actually booked that day (e.g. a virtual studio) so a real lesson is never hidden. Day tabs show every day present in the uploaded week (Monday through Sunday) with that day's actual calendar date. Each lesson box always shows instrument, teacher, and student name (space permitting — a hover tooltip covers boxes too short to fit all three).

### Room Plan
A Richmond-only, printable "where's the teacher" wall chart, next to Room Detail in the nav. Same per-room time grid, but each teacher's bookings in a room collapse into one colored block spanning their actual first-lesson-start to last-lesson-end that day — one continuous block even across a gap in their schedule, not split into pieces. The block itself is a pale tint of the lesson/class color; the darker full-strength color is layered in only over the sub-ranges actually booked, so idle time within a teacher's span reads visibly lighter than busy time. No student names or lesson items — this view only answers "whose room is this and roughly when."

Two print formats, each with an "all week" or single-day picker:
...
| Master Scheduler Report | `Date, Item, From, To, Day, Duration, Facility, Site, Instructor, PL Student, Type, Start Date, End Date` | Room Detail, Room Plan, Daily Digest, room registry, room-linking |
...
Master Scheduler's own `Day` column isn't trusted as-is — it can disagree with that same row's `Date` column (seen on real exports, apparently tied to recurring bookings), so every event's weekday is derived from `Date` instead (see the 2026-09-03 changelog entry below). `Date` stays the single source of truth for both which day a row is bucketed under and which calendar date is shown for it.
...
**2026-09-03** — Fixed Room Schedule and Daily Digest sometimes showing the same calendar date under two different weekday tabs (e.g. both Monday and Thursday reading "8/31"), reported after uploading a real Mon 8/31–Sun 9/6 Master Scheduler export.
- **Root cause:** `mapMasterScheduleRows()` took each row's weekday straight from the report's own `Day` text column, treated as independent from that same row's `Date` column. Both `rsDateForSiteDay()` (the date shown next to a weekday tab) and every day-bucketing filter in Room Schedule/Daily Digest key off `.day`, so any row where ASAP's `Day` and `Date` disagree gets bucketed under the wrong weekday — and if that row happens to be the first one `rsDateForSiteDay()` finds for that (wrong) bucket, its `Date` gets displayed there too, duplicating whatever real date already legitimately owns that same `Date` value under its correct bucket. Confirmed this actually happens in the real sample Master Scheduler file — 2 of 1214 rows have a `Day`/`Date` mismatch, not a one-off.
- **Fix:** added `isoToShortDay()`, deriving a row's weekday directly from its parsed `Date` (via `Date.UTC` + `getUTCDay()`, avoiding the local-timezone drift `new Date(iso)` parsing is prone to) rather than reading the report's separate `Day` column at all — falling back to that column only if `Date` itself fails to parse. `Date` is now the single source of truth for both which weekday a row is bucketed under and which date displays for that bucket, so the two can never disagree again. Confirmed every consumer of `.day` on Master Scheduler events (Room Schedule, Daily Digest, both their print paths) only ever uses it as an opaque bucket key, never displays the raw text — safe to swap what populates it.
- Verified against the real sample file: re-derived every row's weekday from its `Date` and diffed against the original `Day` column (found the same 2 real mismatches noted above, both now correctly rebucketed), confirmed zero rows dropped (1214 in, 1214 mapped), and confirmed `rsDateForSiteDay()` now returns 7 distinct dates for Richmond's 7 weekday tabs with no duplicates. Also tested a synthetic row with a deliberately mismatched `Day`/`Date` pair to confirm the fix rebuckets it correctly rather than just happening to pass on real data.

**2026-09-04** — Split Room Schedule into two views and restored student names, per the user's request:
- **Room Detail** (renamed from Room Schedule): every lesson box now always shows instrument, teacher, and student name once the box is tall enough (student is a new fourth tier alongside the existing item/instructor/time-range tiers); a hover tooltip still covers boxes too short to fit all three. Student names had been dropped from display entirely in an earlier session even though the underlying data already carried them (`mapMasterScheduleRows()`'s `student` field flows through untouched) — no reupload needed, just a rendering change. Print controls removed from this view; it's screen-only now.
- **Room Plan** (new view, next to Room Detail in the nav): a Richmond-only, printable "where's the teacher" wall chart. Each teacher's bookings in a room collapse into one block spanning their actual first-lesson-start to last-lesson-end (`rpTeacherBlocksForRoom()`) — always one continuous block even across a gap in the middle of their day, never split. The block renders as a pale tint of the existing lesson/class color; the darker full-strength color layers in only over the sub-ranges actually booked (`rpBuildGrid()`), so idle time within a teacher's span reads visibly lighter than busy time. No student names or lesson items on this view. The two print formats (17×11 Fixed Rooms, 11×8.5 Booked Only) moved here from Room Detail and were rebuilt on top of these blocks instead of individual lessons — and now print in full color rather than Room Detail's old ink-light black-and-white style, since the color coding is the point of a block chart.
- Verified: JS syntax parses cleanly, every `getElementById` reference resolves to a real element, and CSS braces balance. Did not click through the live UI end-to-end — the app requires a Supabase login not available in this session — so a manual pass in the browser (especially checking block coloring and the two print formats) is still worth doing before relying on this.
